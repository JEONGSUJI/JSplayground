...
const method = (func, url, f, payload) => {
...
const addTodo = ({ target, keyCode }) => {
  ...
  method('POST', '/todos', render, { id: maxId, content, completed: false });
};

const removeTodo = ({ target }) => {
  if (!target.classList.contains('remove')) return;
  const id = +target.parentNode.id;
  method('DELETE', `/todos/${id}`, render);
};

const checkTodo = ({ target }) => {
  const id = +target.parentNode.parentNode.id;
  const completed = !todos.find((todo) => todo.id === +id).completed;
  method('PATCH', `/todos/${id}`, render, { completed });
};

window.onload = () => {
  method('GET', '/todos', render);
};
